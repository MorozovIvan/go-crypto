	...
	return client.Run(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "SESSION_PASSWORD_NEEDED") {
				...
				A := s.modPow(algo.G, new(big.Int).SetBytes(a), algo.P)
...
func (s *TelegramService) modPow(base, exp, modulus *big.Int) *big.Int {
...
func (s *TelegramService) calculateM1(algo *tg.PasswordKdfAlgoSHA256SHA256PBKDF2HMACSHA512iter100000SHA256ModPow, password string, a []byte, A *big.Int) []byte {
	// Calculate k = H(N, g)
	h := sha256.New()
	h.Write(algo.P.Bytes())
	h.Write([]byte{byte(algo.G.Int64())})
	k := new(big.Int).SetBytes(h.Sum(nil))

	// Calculate u = H(A, B)
	h.Reset()
	h.Write(A.Bytes())
	h.Write(algo.Salt1) // B is stored in Salt1
	...
	h.Reset()
	h.Write(ngxor)
	h.Write([]byte(s.phone)) // Using phone number as username
	h.Write(algo.Salt1)
	h.Write(A.Bytes())
	h.Write(algo.Salt1) // B is stored in Salt1
	h.Write(S.Bytes())
