  ...
  methods: {
    ...
    async connectTelegram() {
      ...
      try {
        ...
        const authWindow = window.open(data.auth_link, '_blank', 'width=600,height=600')
        
        // Listen for the callback
        window.addEventListener('message', async (event) => {
          if (event.origin === 'http://localhost:8080' && event.data.type === 'telegram_auth') {
            if (authWindow) {
              authWindow.close()
            }
            ...
            if (event.data.username) {
              this.telegramUsername = event.data.username
            }
            
            // Fetch groups after successful authentication
            try {
              const groupsResponse = await fetch(`http://localhost:8080/api/telegram/groups${this.userId ? `?user_id=${this.userId}` : ''}`)
              ...
              this.error = 'Failed to fetch groups after authentication'
            }
          }
        })
        
        // Start polling as a fallback
        this.pollTelegramStatus(authWindow)
      } catch (err) {
