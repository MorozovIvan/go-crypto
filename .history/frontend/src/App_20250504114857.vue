...
export default {
  ...
  methods: {
    ...
    async connectTelegram() {
      ...
      try {
        // Get the default phone number from the backend
        const response = await fetch('/api/telegram/phone')
        if (!response.ok) {
          throw new Error('Failed to get default phone number')
        }
        const data = await response.json()
        const defaultPhone = data.phone

        // Prompt for phone number with default value
        const phone = prompt('Please enter your Telegram phone number (with country code, e.g., +1234567890):', defaultPhone)
        if (!phone) {
          throw new Error('Phone number is required')
        }

        // Start authentication process
        const authResponse = await fetch(`/api/telegram/auth/callback?phone=${encodeURIComponent(phone)}`)
        if (!authResponse.ok) {
        ...
        // Verify the code
        const verifyResponse = await fetch('/api/telegram/auth/verify', {
          method: 'POST',
          headers: {
        ...
        if (!verifyResponse.ok) {
          ...
          if (errorData.error && errorData.error.includes('2FA password is required')) {
            ...
            const verify2FAResponse = await fetch('/api/telegram/auth/verify', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
        ...
        const groupsResponse = await fetch('/api/telegram/groups')
    ...
    async pollTelegramStatus(authWindow) {
      ...
      const poll = async () => {
        ...
        try {
          const response = await fetch(`/api/telegram/groups${this.userId ? `?user_id=${this.userId}` : ''}`)
          if (response.ok) {
            const data = await response.json()
