...
func (s *TelegramService) AuthenticateUser(phone string) error {
	...
	return client.Run(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			...
			case strings.Contains(err.Error(), "PHONE_NUMBER_INVALID"):
				return fmt.Errorf("invalid phone number format. Please check the number and try again")
			case strings.Contains(err.Error(), "PHONE_CODE_INVALID"):
				return fmt.Errorf("invalid verification code. Please check the code and try again")
			case strings.Contains(err.Error(), "PHONE_CODE_EXPIRED"):
				return fmt.Errorf("verification code has expired. Please request a new code")
			case strings.Contains(err.Error(), "FLOOD_WAIT"):
				// Extract wait time from error message if possible
				waitTime := "24 hours" // Default wait time
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "SESSION_PASSWORD_NEEDED") {
				...
				if !settings.HasPassword {
					...
					return fmt.Errorf("2FA is not enabled for this account. Please enable 2FA in the Telegram app first")
				}
				...
				if err != nil {
					...
					switch {
					case strings.Contains(err.Error(), "PASSWORD_HASH_INVALID"):
						return fmt.Errorf("invalid 2FA password. Please check your password and try again")
					case strings.Contains(err.Error(), "PHONE_PASSWORD_FLOOD"):
						return fmt.Errorf("too many failed 2FA attempts. Please wait 24 hours before trying again")
					case strings.Contains(err.Error(), "FLOOD_WAIT"):
						waitTime := "24 hours" // Default wait time
						if strings.Contains(err.Error(), "FLOOD_WAIT_") {
							parts := strings.Split(err.Error(), "FLOOD_WAIT_")
