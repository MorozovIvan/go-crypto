...
func (s *TelegramService) GetUserGroups(userID int64) ([]map[string]interface{}, error) {
	...
	if err := s.client.Run(ctx, func(ctx context.Context) error {
		...
		// If we have a phone number and code, try to sign in
		if s.phone != "" && s.code != "" {
			phone := s.phone
			code := s.code
			s.phone = "" // Clear after use
			s.code = ""  // Clear after use

			// Create a flow for phone number authentication
			flow := auth.NewFlow(
				auth.PhoneAuthenticatorFunc(func(ctx context.Context, sentCode *tg.AuthSentCode) (string, error) {
					return code, nil
				}, noSignUp{}),
