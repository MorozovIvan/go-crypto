...
func (s *TelegramService) SetAuthCode(code string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.code = code
}
...
func (s *TelegramService) GetUserGroups(userID int64) ([]map[string]interface{}, error) {
	...
	if err := s.client.Run(ctx, func(ctx context.Context) error {
		...
		if s.code != "" {
			code := s.code
			s.code = "" // Clear the code after use

			// Create a custom authenticator that implements UserAuthenticator
			authenticator := auth.UserAuthenticatorFunc(func(ctx context.Context, sentCode *tg.AuthSentCode) (auth.UserInfo, error) {
				return auth.UserInfo{
					Phone:    "", // Phone number will be requested by the flow
					Code:     code,
					Password: "", // 2FA password if needed
				}, nil
			})

			// Create a flow for phone number authentication
