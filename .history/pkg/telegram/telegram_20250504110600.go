...
func (s *TelegramService) AuthenticateUser(phone string) error {
	...
	return client.Run(ctx, func(ctx context.Context) error {
		...
		// First, try to get the current authorization state
		state, err := api.AuthGetState(ctx)
		if err != nil {
			return fmt.Errorf("failed to get auth state: %v", err)
		}

		// If we're already authorized, we need to log out first
		if _, ok := state.(*tg.AuthStateAuthorized); ok {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "SESSION_PASSWORD_NEEDED") {
				...
				_, err = api.AuthCheckPassword(ctx, &tg.InputCheckPasswordSRP{
					...
					A:     []byte(password), // This is a simplified version, in reality we need to compute SRP parameters
				})
				if err != nil {
