...
func (s *TelegramService) GenerateAuthLink() string {
	...
	return fmt.Sprintf("https://oauth.telegram.org/auth?bot_id=%s&origin=%s&return_to=%s/auth/callback",
		botID,
		"http://localhost:5173",
		"http://localhost:5173")
}
...
func (s *TelegramService) GetUserGroups(userID int64) ([]map[string]interface{}, error) {
	...
	if err := s.client.Run(ctx, func(ctx context.Context) error {
		...
		if !status.Authorized {
			// If not authorized, we need to start the bot authentication flow
			if _, err := auth.Bot(ctx, s.botToken); err != nil {
				return fmt.Errorf("failed to authenticate bot: %v", err)
			}
		}
		...
		// Get bot's commands to verify it's working
		_, err = api.BotsGetBotCommands(ctx, &tg.BotsGetBotCommandsRequest{
			Scope: &tg.BotCommandScopeDefault{},
		})
		if err != nil {
			return fmt.Errorf("failed to get bot commands: %v", err)
		}

		// Get updates to see which chats the bot is in
		updates, err := api.UpdatesGetState(ctx)
		if err != nil {
			return fmt.Errorf("failed to get updates state: %v", err)
		}

		// Get the bot's username
		botInfo, err := api.UsersGetUsers(ctx, []tg.InputUserClass{&tg.InputUserSelf{}})
		if err != nil {
			return fmt.Errorf("failed to get bot info: %v", err)
		}

		if len(botInfo) > 0 {
			if user, ok := botInfo[0].(*tg.User); ok {
		...
		// If we get an auth error, clear the client so it will be recreated next time
