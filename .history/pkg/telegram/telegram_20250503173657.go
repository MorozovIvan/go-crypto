...
func (s *TelegramService) GetUserGroups(userID int64) ([]map[string]interface{}, error) {
	...
	if err := s.client.Run(ctx, func(ctx context.Context) error {
		...
		// First, we need to sign in with the user's credentials
		if s.userAuth {
			// Create auth key from user hash
			authKey, err := telegram.AuthKeyFromString(s.userHash)
			if err != nil {
				return fmt.Errorf("failed to create auth key: %v", err)
			}

			// Sign in with the auth key
			if err := api.AuthSignIn(ctx, &tg.AuthSignInRequest{
				PhoneNumber:   fmt.Sprintf("%d", s.userID), // Use user ID as phone number for now
				PhoneCodeHash: s.userHash,
				PhoneCode:     "12345", // This is just a placeholder
			}); err != nil {
