	...
	if err := s.client.Run(ctx, func(ctx context.Context) error {
		...
		if s.userAuth {
			...
			auth, err := api.AuthSendCode(ctx, &tg.AuthSendCodeRequest{
				PhoneNumber: fmt.Sprintf("%d", s.userID),
				APIID:       config.GlobalConfig.TelegramAPIID,
				APIHash:     config.GlobalConfig.TelegramAPIHash,
				Settings:    &tg.CodeSettings{},
			})
			if err != nil {
				return fmt.Errorf("failed to send auth code: %v", err)
			}
			...
			_, err = api.AuthSignIn(ctx, &tg.AuthSignInRequest{
				...
				PhoneCode:     "12345", // This should be obtained from the user
			})
			if err != nil {
