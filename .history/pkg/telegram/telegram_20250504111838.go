	...
	return client.Run(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "SESSION_PASSWORD_NEEDED") {
				...
				currentPassword, err := api.AccountGetPassword(ctx)
				if err != nil {
					s.logger.Error("Failed to get current password", zap.Error(err))
					return fmt.Errorf("failed to get current password: %v", err)
				}

				// Get password settings
				passwordSettings, err := api.AccountGetPasswordSettings(ctx, &tg.InputCheckPasswordSRP{
					SRPID: currentPassword.SRPID,
					A:     []byte(password),
					M1:    []byte(password),
				})
				if err != nil {
				...
				_, err = api.AuthCheckPassword(ctx, &tg.InputCheckPasswordSRP{
					SRPID: currentPassword.SRPID,
					A:     passwordSettings.NewAlgoSalt1,
					M1:    passwordSettings.NewAlgoSalt2,
				})
				if err != nil {
