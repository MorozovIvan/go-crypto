	...
	return client.Run(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "SESSION_PASSWORD_NEEDED") {
				...
				// Get SRP parameters
				srpParams, ok := settings.NewAlgo.(*tg.PasswordKdfAlgoSHA256SHA256PBKDF2HMACSHA512iter100000SHA256ModPow)
				if !ok {
					return fmt.Errorf("unsupported password algorithm")
				}

				// Generate SRP parameters
				srp := &tg.InputCheckPasswordSRP{
				...
				_, err = api.AuthCheckPassword(ctx, srp)
				if err != nil {
