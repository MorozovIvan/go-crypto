...
func (s *TelegramService) GetUserGroups(userID int64) ([]map[string]interface{}, error) {
	...
	if err := s.client.Run(ctx, func(ctx context.Context) error {
		...
		// Get bot's chat memberships using getMyCommands
		api := s.client.API()

		// Get bot's information
		botInfo, err := api.BotsGetBotInfo(ctx, &tg.BotsGetBotInfoRequest{
		...
		// Get bot's commands
		commands, err := api.BotsGetBotCommands(ctx, &tg.BotsGetBotCommandsRequest{
		...
		// Get bot's chat memberships
		chats, err := api.MessagesGetAllChats(ctx, &tg.MessagesGetAllChatsRequest{
			ExceptIds: []int64{},
		})
		if err != nil {
			...
			for _, chat := range c.Chats {
				...
				case *tg.Channel:
					result = append(result, map[string]interface{}{
						...
						"type":        "channel",
						"description": "", // Need additional API call to get description
					})
				case *tg.Chat:
					result = append(result, map[string]interface{}{
						"id":          ch.ID,
