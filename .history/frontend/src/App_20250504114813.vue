  ...
  methods: {
    ...
    async connectTelegram() {
      ...
      try {
        ...
        const response = await fetch('http://localhost:8080/api/telegram/phone')
        if (!response.ok) {
        ...
        const authResponse = await fetch(`http://localhost:8080/api/telegram/auth/callback?phone=${encodeURIComponent(phone)}`)
        if (!authResponse.ok) {
